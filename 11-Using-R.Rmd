...
a name. The base R equivalent of a tibble is called a ***data frame***. In 
most applications, tibbles and data frames are interchangeable, but tibbles
...
print(EmpData)
```
We can also see the whole data set by using the `view()` function.
...
```{r}
print(EmpData[1,])
print(EmpData[1,"LFPRate"])
print(EmpData[EmpData$LFPRate < 0.61,c("MonthYr","UnempRate")])
```
...
# We can use APPLY to calculate them for every column
...
count(simdat,zval)
# COUNT and CUT_WIDTH create a binned frequency table
