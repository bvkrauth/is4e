...
This is convenient and simple to use in many applications, but it can be a
disaster in complex projects.
...
3. Your **results**
   ...
   - You can regenerate ypur results at any time by running your code.
...
Some other applications simply drop missing data from the calculation. This is
...
to exclude them from the calculation by adding the `na.rm = TRUE` option to
many of the statistical functions we use.
...
R has a family of built-in functions for each commonly-used probability
distribution.  
...
- The first expression/line calls `ggplot()` to set up the basic characteristics
  of the graph:
...
You can use `labs()` to add titles, subtitles, etc.  You can also use
...
You can change the color of any geometric elementusing the `col=` argument.
