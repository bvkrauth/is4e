...
to use R to [read and view data](#reading-and-viewing-data-in-r). We are now
ready to start using R with real data.

In this chapter, we will use this knowledge to do further cleaining and analysis
of the Canadian employment data we have already worked with.  We will also use
ggplot, a powerful graphing system that is part of the Tidyverse package.
...
The pipe operator is unusual because it doesn't allow you to *do* anything
new.  Instead, it makes your code easier to read. To see how it works, I will
...
allows us to change or add variables.
...
Suppose we want to create versions of **UnempRate** and **LFPRate** that
...
*new* data table.  In order to actually change the original data table, we
will need to use the assignment (`<-`) operator.
...
EmpData <- EmpData %>%
  ...
  mutate(LFPPct = 100*LFPRate)
```
We can confirm that now we have changed `EmpData`: **MonthYr** is a date variable,
and the variables **UnempPct** and **LFPPct** are part of the table.
```{r ConfirmPermanence}
print(EmpData)
```
:::
...
Suppose we want to know more about the months in our data set
...
Similarly, R has a function called `select()` that allows you to choose
specific variables.
...
Suppose that we want to show months in descending order by
...
are all mixed in together in a single file or even a single worksheet.
This is convenient and simple to use in many applications, but it can lead
to disaster in more complex projects.
...
vector.  There are also functions to calculate variance, standard deviation,
...
to exclude them from the calculation by adding the `na.rm = TRUE` optional
argument.
...
cannot calculate annual growth rates in the first year of our data. So we
can exclude the missing values.
...
tibble. We could just call `mean()` for each of them, but there is
a quicker way.
...
We can construct simple frequency tables for discrete variables
...
The ggplot package has capabilities well beyond what R's built-in commands can
do, or what Excel can do.
...
We can start by making a histogram of the unemployment rate, much like the
one we made earlier in Excel
```{r ggplotHistogram}
ggplot(data = EmpData,
...
- The first expression/line calls `ggplot()` to set up the basic structure
  of the graph:
    - The `data` argument tells R which data set (tibble) will be
      used
    - the `mapping` argument describes the main ***aesthetics***
...
- The rest of the command is one or more statements separated by 
  a `+` sign.  These are called ***geometries*** and are graph
...
- We have used color to differentiate the two lines, but there is no legend 
  or label to tell the reader which line is which.  We will need to fix that.
- The vertical axis is labeled **UnempPct** which is the name of one of
  the variables but not the other. We will need to fix that.
:::
...
[multiple variables](#multivariate-data-analysis).  The capabilities
of ggplot for constructing graphical elements like scatter plots and curve fits
will be well beyond what you can do in Excel, and will hopefully justify
your investment in learning R.
