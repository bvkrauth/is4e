...
1. Review course educational goals
2. Establish course expectations
3. Gather resources and tools, including all needed computer software.
:::
...
- Both Google and the major social media companies are based on providing
  ...
  be directly or indirectly sold to other businesses.
- Amazon and other retailers use what is called A/B testing to fine-tune product
  descriptions and set prices so as to maximize profits.

Some of this data analysis is done by statisticians or computer scientists,
but much of it is done by economists. For example, Amazon is the
second-largest employer of PhD economists in the US 
(after the Federal Reserve System). The average economist does not
...
graduate degree - but it is a first step in that direction. So you should take
...
2. You can do high-school level math including algebra and basic set theory
   and have taken or are currently taking an introductory calculus course.
   - I will not ask you to take derivatives or solve integrals
   - But I will refer to concepts like functions, sequences and limits.
   - The math review appendix provides material and practice problems if you
     need to review these concepts and tools.
3. You have access to a desktop or laptop computer, and basic computer skills.

This is not a textbook on introductory economics, high school math, or
computer skills. If you are a little behind in those skills, be sure to
...
course  in econometrics. In those courses, you will learn more advanced techniques
such as linear regression, you will use statistical packages such as R more
...
available at the course Canvas site https://canvas.sfu.ca/courses/73884.
:::
...
and macOS. Alternatives to Excel include Google Sheets and Apple Numbers.  We will
use Excel here, but the basic skills are transferable to any spreadsheet program
including these alternatives.
...
We will also use a more specialized statistical
...
SAS and Stata, as well as the (free) general-purpose programming
language Python.
...
of packages available; we will use a particularly popular and
...
4. Enter `library(tidyverse)` in the console window.  
   ...
   - If you don't get an error message (it's OK if you get some message
     about "Conflicts"), the installation worked.
