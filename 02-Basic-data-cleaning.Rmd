...
1. Identify and implement the principles of tidy data.
2. Identify and implement the principles of reproducible research, including:
   - Data documentation
   - Version control
2. Identify and work with ID variables.
...
installed. Refer to the [installation instructions](#installing-microsoft-excel) as
needed.
...
2. Keep an unmodified copy of all original data files.
   - Avoid changing the file name if possible.
   - Use folders and your `README` file to clarify the contents of each file.
...
4. Give files and variables brief but informative names.
   - Make your names ***portable***, i.e., easily moved across operating
     systems, file formats, and applications.  For example, stick to letters
     and numbers, and avoid using spaces or special characters.
   - Following a naming convention may help make your names more informative
5. Use ***version control*** to manage different versions of your work.
   - Start your project by creating a ***working copy*** of every file you plan
     to edit.
     - There should only be one working copy of each file.
     - You can edit the working copy.
   - Each time you complete a major step in your project, make your current
     working copy the ***master version***.
     ...
     - The master version should not be directly edited, but it can be replaced
       with the most recent working copy.
...
6. Each observation has a unique value for some **unique identifier** or **ID**
   variable.
...
"Québec" and "Nova Scotia" and a **ProvinceID** variable that takes on values like "QUEBEC"
and "NOVASCOTIA."
...
names and other information such as year of birth.  Matches made this way are
...
data. Another is to use a [digital object identifier](https://www.doi.org/)
or ***DOI*** if one is available. DOI is an international system that allows
participating organizations to provide persistent links to web pages or other
files. The *Sources* worksheet above provides both regular and DOI addresses.
...
- The DOI URL (https://doi.org/10.25318/1410028701-eng) also tells us where a
  ...
  Instead it is a link that (currently) redirects the user to the address
...
Canada will (hopefully) update the DOI entry with the new URL.
:::

### Cell formatting {#cell-formatting}

In Excel, we distinguish between the contents of a cell and how it is displayed.
...
regularly use productivity applications like Microsoft Word.  You select
the cells you are interested in modifying, then you go through the
menus.
...
As mentioned in the section on [reproducibility](#data-management), we
will want to save a working copy of our file before proceeding.  In
addition, it is wise to save your work frequently.

::: example
**Saving a working copy**

To save a working copy of your file:

1. Select `File > Save As` from the menu.
   - You need to select `Save As` rather than `Save` so you can change the file
     name.
2. Follow the usual steps in the dialog box to save a file.
   - Choose a new file name, and a convenient location.
   - I would choose the file name  `CanEmpNov20_WC.xlsx`.to make clear that this
     file is my working copy.
:::
...
column A.  We will fill in column A shortly.
:::
...
on [reproducibility](#data-management), the next step is to:

1. Check the file for mistakes and incomplete work.
2. Update the master version to the current working copy.

::: example
**Updating the master version**

We do not yet have a master version, so we will need to create one.

1. Select `File > Save As` to save the file under a new name.
2. Follow the usual steps to save the file under a new name.
   - I would choose `CanEmpNov20Clean.xlsx`.

You can compare your master version to my version at
