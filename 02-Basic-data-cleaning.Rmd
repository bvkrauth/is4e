...
is by working on a real data project, soe will also apply this knowledge by
...
3. Use basic Excel terminology and concepts including:
   ...
   - Cell ranges
4. Use fill and series to enter new data.
5. Use Excel formulas to construct new variables.
6. Use relative and absolute cell addresses in Excel formulas.
7. Use logical and text data in Excel.
8. Use dates in Excel.
:::
...
interface by name. You can use these elements for years without ever
learning their names, so I will list them here for your reference:
...
- There are several items just below the ribbon that we will learn to
...
- Most of the screen displays a rectangular grid of ***cells*** that is
  called a ***spreadsheet*** or ***worksheet***:  
...
Open the file
[https://bookdown.org/bkrauth/BOOK/sampledata/CanEmpNov20.xlsx](sampledata/CanEmpNov20.xlsx)
in Excel.  You can do this by either double-clicking on it, or by using
`File > Open` from Excel's menu.  The file should look something like this:
...
be ***reproducible***. Reproducibility means that an interested reader
should be able to figure out exactly how you got your results, 
and should be able to generate those results themselves.

The "interested reader" might be *you*, as it is common to return to an
...
what it means.  The interested reader might also be your coworkers, your boss,
or your clients.
...
2. Keep your data and analysis projects as ***portable*** as possible. Data is
...
4. Use ***version control*** to manage and track changes to your files as you
   work. Version control systems can be very elaborate, but I will teach you a
   simple folder-based workflow:
   - Create a ***working copy*** of each file you plan to edit.
     - There should only be one working copy of each file.
   ...
   - Each time you complete a step in your project, make your current
     ...
     - There should only be one master version of each file.
     - The master version represents the project at its most-recently-completed
       stage. It should not include any half-finished work.
...
Software developers and professional data analysts use formal version control
systems like [GitHub](https://github.com/). GitHub tracks every change ever made
to a group of files, and can allow multiple users to work together on a project.
It is based on a model in which each user has their own working copy, and
can commit (or propose) changes to the master version, along with an explanation
of the changes.  You can see my GitHub page at https://github.com/bvkrauth.

GitHub takes a little time to set up and learn, but is an extremely useful tool
if you do a lot of coding or data analysis.
:::
...
6. Each observation has a unique value for some ***unique identifier*** or
   ***ID*** variable.
...
Anoter strategy for addressing link rot is to use a
...
we typically avoid using proper names as ID variables becuase they are not
...
2. **ProvAbb**: a standardized (text) code based on the two-letter
   postal abbreviation.
...
example, each student in BC grades K to 12 is assigned an ID number called
a PEN while each person in the healthcare system is assigned a
different ID number called a PHN. There is no direct way to match PEN and PHN,
so we have to match education and health records on a combination of proper
names and other information such as date of birth.  Matches made this way are
called "probabilistic" matches, meaning (roughly) that the records probably
describe the same person but might not.
:::

### Planning {#cleaning-plan}

Before diving into cleaning our data, it is often wise to look closely at the
data and construct a  ***cleaning plan***. Here is a simple template:
...
The cleaning plan should be based around what we plan to do with the data, but
...
We are now ready to start using Excel to active clean our data. We will also
introduce a set of important Excel tools and concepts that are useful in
cleaning data. Be sure to use your working copy of the data, and to save
the file after completing each task so that you don't lose your work.
...
We can solve this problem by making column B wider:
...
Excel has several built-in tools for finding information on the names and
arguments of its functions. Search engines such as Google can also be helpful.
...
- *Google*
  ...
  this, the top recommendation is for Microsoft's online documentation for the
  `LN()` function. Read the documentation, which says that the `LN()`function
  takes a single numeric argument and returns the natural logarithm of that
  argument.
...
This is exactly what we would want; each cell in column H
calculates the province's labour force by adding the unemployed
count (column D) and employed count (column E) from the same province
(same row).  So cell H2 should contain `=D2+E2` and cell H3 should contain
`=D3+E3`.

Similarly, the formulas in columns I (**LFPRate**), J (**UnempRate**),
and K (**LogPop**) have all copied exactly as we would would want them
to be.
:::
...
In this case, that is not what we want.  We want  *all* of the cells to
contain the total population of all provinces, which means all of them
should contain the formula `=SUM(C2:C11)`.
:::

We can tell Excel to treat a given cell reference as absolute by
adding the `$` character in front of the reference. For example,
...
which takes two arguments: the value to rank, and the range of values
to use for the ranking.  We want the first argument (the province's population)
to vary across observations, but we want the range of values (the populations of
...
As you can see, Excel displays the correct ranks. If you aren't sure how
this works, look at the contents of each cell of column M to see how the
formula copied from cell M2.
:::
...
Excel has various specialized tools for entering, storing and processing
these data types.
...
**UnempRate** and **LFPRate** are both calculated in proportional units
(a number between 0 and 1). But we might prefer to display them 
in percentage units (a number between 0 and 100).

We *could* do this by changing the formulas (for example, change the `LPFRate`
formula to `=100*H2/C2`).  An easier approach is to change the cells'
...
2. Which of the following characteristics is necessary for a variable to
...
3. For each of the following Excel formulas, suppose we copy that from cell
...
4. Construct an Excel formula to do each of the following.
