...
1. Have a ***working copy***, which is the file you are actively working
   on right now.
   - There is only one working copy of a given document
     (in a simple version control system).
   - You can edit the working copy.
2. Have a ***master version***, which is the most recently saved file that
   is "complete."
   - There is only one master version (in any version control system).
   - Do not edit the master version.  Instead:
   ...
   - You should try to work in small and distinct projects (like adding a single
     variable or group of related variables).
3. As needed, you can keep any number of ***archived versions***, which are
   earlier master versions (or occasionally earlier working copies) that
   you wish to keep around for some reason.
   - There can be many archived versions.
   - Never edit an archived version.
   - Archiving strategies vary depending on the importance/sensitivity
     of the project.
