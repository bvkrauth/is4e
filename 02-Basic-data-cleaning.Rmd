...
In later chapters, we will use Excel to perform a statistical analysis of
several economic data sets.  But before we can analyze data, we usually need
to ***clean*** it. Cleaning data means putting it into a form that is ready to
analyze, and can include:

- Converting data files from other formats into Excel format.
- Reorganizing complex tables of data into simple tables
- Combining data across multiple tables
- Creating new variables, or modifying existing variables

In this chapter we will learn basic Excel tools and some principles of
data cleaning.  We will also apply this knowledge by using Excel 
to clean a real data set.
...
1. Document the source of all original data.
...
not remember where you found the data, what you have done with it,
or what it means.

### Tidy data {#tidy-data}

To keep analysis easy and minimize errors, data should be organized
as what data scientists sometimes call ***tidy*** data. Tidy data 
has the following properties:

- Data is arranged in one or more simple rectangular grids or ***tables***.
- Each row in a table (after the top row) represents a distinct
  ***observation*** or ***case***.
...
- The top row of a table should give an informative and unique name for 
  each variable.
- One variable (ideally the first column) in a table serves as a 
...
## A quick tour of Excel {#a-quick-tour-of-excel}
...
The rest of this chapter will analyze November 2020 employment data 
for Canadian provinces. You can download the data file at https://bvkrauth.github.io/files/CanadaEmploymentNov2020.xlsx. 
...
When looking through the data, one should be thinking ahead
about what needs to be done to clean it.

::: example
**A plan for cleaning the employment data**

The first thing to consider is whether we have a unique ID variable and
to create one if we don't. In our employment data, the province name
...
The second set of modifications to consider is whether there
...
Before we start cleaning our data, we will first learn a few 
tools for changing how our data is displayed. These tools
do not change the data itself.

***Sorting*** allows us to re-order the rows of our data based on 
the value in one or more of the columns.
...
***Freezing panes*** allows us to keep some rows and/or columns 
visible regardless of which cell is currently selected.  For example,
it will allow us to keep the top row of variable names visible no
matter which row of data we are looking at.
...
You can change many aspects of how a cell displays, including: 
...
The procedure for doing this is straightforward if you regularly use
...
but it might be more convenient to use the two-letter postal abbreviation
instead:
