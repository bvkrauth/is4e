...
- *From the menu*: Select any cell in column B, and then select
  `Home > Format > Column Width...`.  A dialog box will appear that allows you
...
- *Auto-fit* (this is what I usually do):  Select *all* of column B by clicking on
  the "B", and then select  `Home > Format > Column Width...`.  You can also do
  this by double-clicking on the line between column headers B and C.
...
3. We have two options for calculating the natural log of cell C2 in this
   ...
   *Built-in Excel tools*: This is probably unnecessary for a function as simple
   ...
   c. Either enter the text `C2` here or select cell C2 with the mouse. The
      dialog box will display the value in cell C2 (`3588700`) as well as the
      calculated value for the log of cell C2 (`15.0933058`).
   d. Select `OK`.
...
we need to know its ***file format***. A file format is just a set of rules
...
platform-independent, human-readable, compact, and (mostly) secure. Every major
statistical analysis application can read CSV files, as can many other
applications.
...
2. *Archive your current master version* by renaming it and moving it to the
   `archives` sub-folder.
