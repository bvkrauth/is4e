...
master version, along with an explanation of the changes.  You can see my GitHub
page at https://github.com/bvkrauth.

Git and GitHub take a little time to set up and learn, but are extremely useful
tools if you do a lot of coding or data analysis.
:::
...
cleaning data. Be sure to use your working copy of the data, and to save the
file after completing each task so that you don't lose your work.
...
text, logical (true/false) values, dates, and times. Excel has various
specialized tools for entering, storing and processing these data types.
...
places than that.  A cell's contents are distinct from its
***numeric display format***, which is how it appears on the screen. We can
change the display format of a cell or group of cells without changing their
contents.
...
***Text data*** is also called ***character data*** or ***string data***. In
Excel, text values can be entered directly in a cell, can be used in a formula,
...
and ***times***.  Dates and times are a surprisingly complex subject that can
create all sorts of problems on a computer, for several reasons:
...
- We want to be able to add and subtract dates and times:
  ...
  - 11:30 pm on December 31, 2020 is 45 minutes before 12:15 am on January 1,
    2021
- We want to handle time zones, daylight savings time, leap years, and even leap
  seconds.
- We want to do this in a way that is perfectly accurate but hides all of these
...
2. Dates are *displayed* according to the cell's display formatting:
   - The default display formatting varies across regions, so the same date in
     the same Excel file might display differently on two different computers.
3. When you enter something into a cell that looks like a date, Excel does
   several things behind the scenes:
   - Excel guesses the date format for what you have entered.
   - Excel converts what you have entered to the internal storage form (days
     since base date).
   - Excel changes the display format to what Excel thinks it should be (based
