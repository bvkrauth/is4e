...
Set differences take elements out of the first set if they are in the second
set:
   $$\{1,2\} - \{3,4\} = \{1,2\}$$
   $$\{1,2,3\} - \{3,4\} = \{1,2\}$$
...
and we will see them regularly in statitics.
...
are a few other ways of defining a sequence:
 ...
 - As a sequence of variables with subscripts: 
   $(x_1,x_2,x_3)$. We will usually use a variable as the subscript $x_i$ when
   we want to talk about an arbitrary element of the sequence.
...
7. Find the values of each of these summations
   ...
   c. Find $\sum_{i \in 1}^\infty x_i I(i < 4)$.
