...
Suppose $x \sim N(2,9)$. To find $Pr(x \leq 1)$, we can standardize $x$ and 
use the standard normal CDF:
\begin{align}
  \Pr(x \leq 1) &= \Phi\left(\frac{1-2}{\sqrt{9}}\right) \\
    &= \Phi(-0.333)
\end{align}
We can then use Excel's `NORM.S.DIST()` function (the formula will be `=NORM.S.DIST(-0.333,TRUE)`) to get $\Pr(x \leq 1) \approx 0.37$.
:::
...
outcome $\omega$. For convenience, I have created a table below that shows every
value of $\omega$ in the sample space, its probability, and the associated values
of $y_{Red}$ and $y_{14}$.
...
\begin{align}
  f_{Red,14}(1,35) &= \Pr(y_{Red}=1 \cap y_{14} = 35) \\
    &= \Pr(\omega \in \{14\}) = 1/37
\end{align}
There are 17 outcomes (count them in the table above) in which red wins and 14 
loses, and each has probability $1/37$:
\begin{align}
