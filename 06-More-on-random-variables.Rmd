...
simdata <- tibble(x = rnorm(100),
                  ...
                  y3 = x)
p0 <- ggplot(data = simdata, mapping = aes(x = x)) +
  geom_point(aes(y=y0),col="blue") +
  xlab("x") +
  ylab("y")
...
In our roulette example, both $y_{Red}$ and $y_{14}$ depend on the original
...
Therefore, the joint PDF of $y_{Red}$ and $y_{14}$ is:
\begin{align}
...
\begin{align}
  f_{red,14}(a,b) &= \begin{cases}
      ...
      \end{cases} \nonumber
\end{align}
Then the marginal PDF of $y_{Red}$ is:
\begin{align}
  f_{red}(a) &= \sum_{b \in \{-1,35\}} f_{red,14}(a,b) \\
    &= f_{red,14}(a,-1) + f_{red,14}(a,35) \\
\end{align}
Plugging in values in the support of $y_{Red}$ we get:
\begin{align}
...
with an infinite number of joint distributions. For example, the last three joint
...
The conditional PDF of the player's net profit from a bet on red given the
net profit for a bet on 14 is:
\begin{align}
  f_{red|14}(a,b) &= \Pr(y_{Red} = a| y_{14} = b) \\
    ...
    &= \begin{cases}
      ...
      \end{cases}
\end{align}
The conditional PDF of the player's net profit for a bet on 14 given the net
profit for a bet on red is:
\begin{align}
...
For example, the first two scatter plots in Figure \@ref(fig:JointIsNotMarginal)
show cases with zero covariance, the third shows a case with negative
covariance, and the fourth shows a case with positive covariance.

::: example
**Calculating the covariance from the joint PDF**

The covariance of $y_{Red}$ and $y_{14}$ is:
  \begin{align}
    cov(y_{Red},y_{14}) &= \begin{aligned}[t]
...
Returning to the scatter plots in Figure \@ref(fig:JointIsNotMarginal), the
first two scatter plots show cases with zero correlation, the third shows a
case with negative correlation, and the fourth shows a case with positive
correlation.  The correlation in the fourth case is exactly equal to one,
...
The first two scatter plots in Figure \@ref(fig:JointIsNotMarginal) show cases
in which the covariance/correlation between $x$ and $y$ is exactly zero.  
But $x$ and $y$ are only independent in the second scatter plot.  In the first
scatter plot, there is clearly some kind of relationship between $x$ and $y$
(in fact $y = x^2$) so they cannot be independent.
...
relationship as in the first scatter plot of Figure 
\@ref(fig:JointIsNotMarginal) above, the covariance will not necessarily
