...
programming language is a lot like learning a regular language: learning the
rules can be helpful, but it is also important to learn by doing.
...
EmpData <- read_csv("sampledata/EmploymentData.csv")
```
As you can see, R guesses each variable's data type, and reports its guess. The
guesses are usually good, but you can override them if not. It is always a good
idea to review this output and make sure that everything is the way that we want
it:
