...
```
R is an application that has many of the same features as Excel, 
but is specially designed for statistical analysis. It is a 
little more complex, but more powerful in many important ways. 
...
2. Look at the to the **History** window in the upper right corner to see
   a full list of recently exectued commands.  You can double-click on 
...
R Mardown is an implementation of the Markdown markup language in R. 
...
included within a ***code fence*** that identifies the text inside 
...
hist(rnorm(100))
```
The "side effect" is often the main purpose of the expression.
...
```{r}
# REP repeats something a particular number of times (like Excel's Fill tool)
ones <- rep(1,times=10)
print(ones)
...
1. Our first step can be acccomplished using the `seq()` function, 
...
Although I will need to test you on specific knowledge, you should also 
