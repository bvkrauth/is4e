...
but more powerful in many important ways.
...
To prepare for this chapter, please ensure that you have R and RStudio
available on your computer. Refer to the 
...
print("Hello world!")
```
R will respond with something like:
```{r HelloWorld2, echo = FALSE}
print("Hello world!")
```
Note that if you are viewing this textbook in a browser, you can copy any block
of code by clicking in its upper right-hand corner.  You can then paste it in
the console window.
:::
...
- RStudio shows pop-ups with helpful information about your command as you type
  it.
- RStudio maintains a ***command history*** that remembers commands you
...
The simplest way of accessing the command history is to press the up-arrow
...
2. Enter a valid R command in the first line of the file, for example
   ...
   print("Hello world!")
   ```
3. Enter another valid R command in the second line of the file,
   for example
   ```{r, eval = FALSE}
   print("Goodbye world?")
   ```
...
You will see the results of both commands in the Console window.
:::
...
producing documents - HTML files (web pages), Microsoft Word documents,
PDF files, etc. - that have R code and analysis embedded in them. In fact, this
book is written in R Markdown.
...
A markup language is a way of writing documents in text files whose content is
directly readable but also can be formatted and displayed (rendered) in a
visually appealing way. The best-known markup language is HTML, which is the
...
2. The default options are fine, so select `OK`. RStudio will create an
   example R Markdown file that you can use as a template. It will look
   something like this:
...
You can run the R code in an R Markdown document in one of two ways. First,
you can run and display results for individual ***chunks*** of code. A
chunk is a few lines of R code surrounded by a "code fence" that serves
to distinguish R code from ordinary text.
...
1. Close RStudio. You may get a warning message that looks something like this:
   ![Save workspace dialog box](bin/RSaveWorkspace.png){width=30%}

   *Never choose "Save" here*, as it would cause R to
...
2. Click on the `Don't Save` button, and you will exit RStudio.
:::
...
In addition, some expressions have a ***side effect***.  That is, they
make something happen like:

  - Cause something to appear on your screen.
  - Create or modify a file on your computer.
  - Change something in R's memory.

You can execute any valid R expression as a command.

:::example
**Expressions**

When you enter an expression as a command, R will usually display the value it
returns.
```{r ExpressionExample}
...
x <- 5
# Display its contents using the print() function
print(x) 
# Display its contents by giving its name
x
```
:::
...
R has hundreds of additional object types, including:
...
These object types are all built from the basic components of atomic vectors,
lists, and attributes. Users can also define their own object
types, and there is an extensive system to support generic functions and
object-oriented programming (if you know what that is).

### Functions and operators

R has hundreds of built-in mathematical and statistical ***functions***, and
users can define their own functions. You have already seen a few.
Their format and usage is quite similar to Excel, but there are some important
differences.
...
2. You can obtain help on any function by entering `?` and its name
   ...
   - Enter `? seq` in the console window, and look in the Help window (you
     can find it in the lower-right panel).
3. Most functions accept one or more ***arguments***.
   ...
   - The `seq` function's arguments include `from`, 
    `to`, `by`, `length.out`, and `along.with`.
...
There are thousands of packages available online.  We will use a
particularly useful package called the ***Tidyverse***.
...
language without causing thousands of existing programs to stop working.
...
Having skimmed the basic features of R, it is time to use it for some real
work.  As with Excel, we start  any data analysis project in R by
opening the data file, viewing it, and constructing a plan to clean it.
...
the name of the CSV file, either as a local file address or a URL.
...
guess.  It is always a good idea to review this output and make sure
...
The `read_csv()` function creates an object called a data table or
***tibble***. A tibble is a Tidyverse object type that describes a table of
tidy data.
...
We can also see the whole table by using the `View()` command or through
RStudio.
...
- We can count the rows and columns with `nrow()` and `ncol()`, or we
...
In a later chapter, we will learn how to view other table properties such
