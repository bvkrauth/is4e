...
- RStudio maintains a ***command history*** that remembers commands you have
  previously entered.  This is useful when you did something a while ago, but
  either don't remember exactly how you did it, or don't want to type it all in
  from the beginning.

The simplest way of accessing the command history is to press the up-arrow key
...
in a browser.  Read the document and compare it to the original R Markdown file
to get an idea of the capabilities of R Markdown.
:::
...
file with a few lines of content is a valid R Markdown file, and like HTML,
Markdown is designed so it still "works" if you do something unexpected.
...
Any object can also have ***attributes***.  This attributes of an object are a
list associated with the object that provides additional information.
...
- our two atomic vectors have attributes NULL. That's R's way of saying they
  have no assigned attributes
...
lists, and attributes. Users can also define their own object types, and there
is an extensive system to support generic functions and object-oriented
programming (if you know what that is).
...
name of the CSV file, either as a local file address or a URL.
...
- The **MonthYr** variable is also stored as `col_character()`.
  ...
  - We will want **MonthYr** to be stored explicitly as a date variable, so we
    make a note of that issue here and will fix it later.
:::
