...
In this chapter, we will use Excel to construct ***univariate*** statistics
and charts, i.e., statistics and charts that describe a single variable. We
will also learn some Excel tools for viewng data.
...
Canada from January 1976 through January 2021.

1. Set up a project folder.
2. Download the file at
[https://bookdown.org/bkrauth/BOOK/sampledata/CanEmpHist.xlsx](sampledata/CanEmpHist.xlsx) and put it in your raw data subfolder.
...
Our main data set for analysis is in the worksheet *Data for Analysis*,
...
Statistics Canada can have hundreds of observations and hundreds of
thousands of variables, and companies and governments often work with
transactions-level data that includes millions of observations.
Humans are smart, but not smart enough to fully understand a large data set
without some kind of simplification or "dimension reduction." Instead of trying
to directly interpret thousands or millions of numbers, we will need to
...
The original data are ordered by date. But suppose
we wanted it to be ordered by unemployment rate (with the highest rate on top)
...
At this point, only the months when Paul Martin was the Prime Minister
...
Go down to row 50 or so in your worksheet. Notice that you cannot
see the variable names in row 1 any more.  We can make sure
the first row stays visible by doing the following:
...
## Univariate statistics in Excel {#univariate-statistics-in-excel}
...
6. The sample ***median*** is a number that is above half of 
   ...
   - The sample median (and any other quantile/percentile) is what statisticians
     call an "order statistic" - it is constructed by sorting observations
     and choosing a particular value in the sorted list.
   - When the number of observations is *odd*, the sample median is easy to
     calculate: for example the sample median of $D_3 = (4,1,3)$ is the
     second-lowest/second-highest value $\hat{m} = 3$.
   - When the number of observations is *even*, definitions of the sample
     median vary.  For example, what is the sample median of $D_4 = (4,1,3,2)$?
     It could be the second-lowest value ($2$), the second-highest value ($3$),
     or the average of the two ($2.5$).
7. The sample 90th percentile is a number that is above 90\% of
   observations and below the other 10\% of observations.
...
3. Select the tab for the *Data for Analysis* worksheet, and 
...
3. Use the `STDEV.S()` function to report the standard deviation 
   ...
   - There is another built-in Excel function called `STDEV.P()` that
...
3. Fill in cell B2 using the Excel function `COUNTIFS()`.
   - The `criteria_range` will be the data in our main data set
     (Data for Analysis worksheet) covering the Party variable 
...
4. We will want to copy the formula in cell B2 to the other cells in
...
6. To do that we just need to divide each cell in the Count column
...
In constructing bins, we need to apply some good judgment, and 
...
4. Update the formula in cell C2 to use absolute references where
   appropriate.
5. Copy the contents of cell C2 to cells C3:C6.
6. Fill in the **Percentage** column (C) with the appropriate formula
...
::: {.fyi data-latex=""}
The `FREQUENCY()` function is another way to create a frequency
...
Another way to explore our data is by ***visualizing*** it: 
...
5. Color and formatting should be used to simplify interpretation 
...
Healy's *Data Visualization* or Cole Nussbaumer Knaflic's 
...
Pie graphs are an alternative way of depicting relative frequencies,
and are also available in Excel along with 3-dimensional variations
on both pie and bar graphs:  
...
pie slices. They are also much better at assessing relative
...
1. Select or double-click on the horizontal axis to open the 
   `Format Axis` box.
2. Select `Axis Options`.
...
You can download
[https://bookdown.org/bkrauth/BOOK/sampledata/CanEmpHistResults.xlsx](sampledata/CanEmpHistResults.xlsx) if you want to check your work against mine. 
:::
...
this chapter by developing tools for
...
2. Consider the following table of data
   ...
   c. Sample variance of age.
   d. Sample standard deviation of age.
