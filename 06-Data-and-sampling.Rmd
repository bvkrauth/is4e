...
- In the second graph, $x$ and $y$ have something of a 
  positive relationship. High values of $x$ tend to go
  with high values of $y$.
- In the third graph, $x$ and $y$ are closely related. In fact, 
  they are equal.

```{r JointIsNotMarginal, echo = FALSE, fig.margin = TRUE, fig.cap = "x and y have the same marginal distribution in all three graphs, but not the same joint distribution."}
x <- rnorm(100)
y <- rnorm(100)
par(mfrow=c(1,3))
plot(x=x,
...
standard deviations are always[^3] positive, 

[^3]: More precisely, either or both of $\sigma_x$ and $\sigma_y$ could be zero.
...
```{r UncorrelatedButNotIndependent, echo = FALSE, fig.margin = TRUE, fig.cap = "x and y are uncorrelated, but clearly related."}
x <- rnorm(100)
plot(x=x,
     y=x^2,
     type="p",
...
2. Use some purely random mechanism[^4] to choose a small subset of cases
...
[^4]: As a technical matter, the assumption of independence requires
...
- They usually have clear *time trends*.  
  - For example, Canada's realGDP has been steadily growing for as 
