...
```{r}
ggplot(data=EmpData,aes(x=UnempPct,y=LFPPct)) + 
  geom_point(aes(col=PrimeMinister))
```
...
In chapter ???, we learned about the covariance and correlation as 
two ways of describing the relationship among random variables. 
...
Both `cov()` and `cor()` can be applied to an entire data set:
```{r}
