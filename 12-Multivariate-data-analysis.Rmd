...
knitr::opts_chunk$set(echo = TRUE,prompt=TRUE,tidy=TRUE,collapse=TRUE)
library(tidyverse)
...
The simplest application of a pivot table is to construct a table of frequencies. By
default, pivot tables report ***absolute frequencies*** - a count of the *number*
...
2. Drag "MonthYr" into the box marked "$\Sigma$ values". The pivot table will now 
   look like this:
...
2. Right-click on the "Count of MonthYr2" column, and select `Value Field Settings...`.
...
1. Click on the ![Row Labels button](bin/RowLabelsButton.png) and the 
...
ggplot(data=RoundedEmpData, aes(x=UnempPct,y=LFPPct)) + 
  geom_point(col="red")
```
...
the aesthetic:
```{r}
ggplot(data=EmpData,aes(x=UnempPct,y=LFPPct,col=Party)) + 
  geom_point()
ggplot(data=EmpData,aes(x=UnempPct,y=LFPPct,col=MonthYr)) + 
  geom_point()
```
...
ggplot(data=EmpData,aes(x=UnempPct,y=LFPPct)) + 
 geom_point(size=0.5) +
  stat_summary_bin(fun='mean', bins=4,
                   col="red", size=1, geom='point') +
  stat_summary_bin(fun='mean', bins=4,
                   col="red", size=0.5, geom='line') +
  stat_summary_bin(fun='mean', bins=20,
                   col="blue", size=1, geom='point') +
  stat_summary_bin(fun='mean', bins=20,
                   col="blue", size=0.5, geom='line') +
  stat_summary_bin(fun='mean', bins=100,
                   col="green", size=1, geom='point') +
  stat_summary_bin(fun='mean', bins=100,
                   col="green", size=0.5, geom='line') +
  geom_text(x=13.8,y=64.1,label="4 bins",col="red") +
  geom_text(x=13.8,y=62.5,label="20 bins",col="blue") +
  geom_text(x=13.8,y=60,label="100 bins",col="green")
```
...
different techiques for smoothing, but they are all based on taking a
