...
analysis in both Excel and R. The statistics and graphs we have learned
can be used to describe the behavior of a single variable.

This chapter will develop some simple techniques for understanding the
relationship *between* two or more variables. Most data analysis in economics
is focused on understanding relationships between variables, so this is an
important topic that will be central to the next statistics course you take.
Here, we will only introduce a few basic ideas and methods.
...
1. Calculate and interpret the sample covariance and correlation.
2. Distinguish between pairwise and casewise deletion of missing
   values.
3. Construct and interpret pivot tables in Excel including
   - Frequency tables
   - Cross-tabulations
   - Conditional averages
4. Construct and interpret scatter plots in R.
5. Construct and interpret smoothed-mean and linear regression plots in R.
:::

To prepare for this chapter, please review the section on
[multiple random variables](#multiple random variables) and previous chapters
on using Excel and R.
...
For the Excel examples we will start with the file
[https://bookdown.org/bkrauth/BOOK/sampledata/EmploymentData.xlsx](sampledata/EmploymentData.xlsx).  This file is similar to the employment data file we used in Chapter 
\@ref(basic-data-analysis-with-excel).

For the R examples we will start with the 
[EmploymentData.csv](sampledata/EmploymentData.csv)
file we used in Chapter \@ref(using-r).  Execute the following R code to get 
started:
...
correlation as defined in Chapter \@ref(more-on-random-variables).

### Covariance and correlation in R

The sample covariance and correlation can be calculated in R using 
the `cov()` and `cor()` functions.  These functions can be applied
to any two columns of data:
...
of a pair of variables.  As you can see:
...
slightly different ways to exclude missing values:
...
generated in this chapter at
[https://bookdown.org/bkrauth/BOOK/sampledata/EmploymentDataPT.xlsx](sampledata/EmploymentDataPT.xlsx) 
...
and each point represents an observation. In some sense, the scatter
plot shows everything you can show about the relationship
between the two variables, since it shows every observation. 
...
We can use color to represent a third variable, whether it is discrete 
or continuous.
...
and adds a line based on 4 bins (orange) and a line based on 100 bins (light
blue).
...
your next econometrics course learning to use it.

## Chapter review {-#review-multivariate-data-analysis}

Econometrics is mostly about the relationship between variables: price
and quantity, consumption and savings, labour and capital, today and 
tomorrow.  So most of what we do is multivariate analysis.  

This chapter has provided a brief view of some of the main techniques for 
multivariate analysis.  Our higher-level statistics courses (ECON 333, 
ECON 334, ECON 335, ECON 433, ECON 435) are all about multivariate analysis,
and will develop both the theory behind these tools and the set of applications
