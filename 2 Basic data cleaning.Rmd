---
title: "Chapter 2: Cleaning data with Excel"
...
In all of these cases, it is important that our analysis is
***reproducible***. Reproducible research provides all information needed to
confirm and possibly extend the results of the research.  Why is
reproducibility important?

- It guards against fraudulent research by making it easier to detect. 
- It guards against errors by making them easier to detect. 
- It allows for researchers to build on research by developing additional
  findings

The key to reproducibility is to carefully maintain a record of all data and
analysis steps leading to every result that is ever reported. This record can
then be shared with others who (hopefully) can then reproduce the results for
themselves. 
...
2. (optional) Rename the working copy if you like.
   - You should choose a brief but clear name. Avoid names like "my file.xlsx."
   - I usually put the current date at the end of my filenames, for example
     "NLSY wage data 200614.xlsx" This allows me to keep earlier versions of a file
     while still knowing which version is the current one.
3. Open the working copy. 

### Maintain a master version

**RULE #4: Maintain a master version, but keep selected intermediate versions as well.** 
...
### Contents versus display format
...
Once you have followed all of the steps in this chapter, you should clean up
your file:

- Undo any undesirable formatting or content changes.
- Fill in any incomplete columns

and save it to your project folder. We will use it again soon.
