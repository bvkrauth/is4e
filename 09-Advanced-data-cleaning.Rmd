...
in cleaning complex data sets: file formats, merging, aggregation, and how to
...
1. Identify and work with common data file formats:
   - Excel native (.xlsx) format
   - Text format (CSV, tab/space delimited, fixed width)
2. Describe and implement common data-cleaning tasks:
   - Matching/merging using the XLOOKUP function
   - Crosswalk tables
   - Aggregation by groups using "IFS" functions
3. Describe and use Excel data-management tools:
   - Error codes
...
and download the following three data files:
...
The third file may display in your browser rather than downloading like the other 
...
The current version of the PWT is available at https://www.ggdc.net/pwt and is
built from two distinct data sources:
...
modern applications can also ***import*** (read) and ***export*** (save) files
in other formats including native formats of other popular programs.
...
- A text file is just a sequence of lines of ***plain text*** characters with no
  ...
  - Most plain text characters are standard visible letters, numbers, spaces,
    and symbols.
...
*scarce* resources. Many well-known applications have moved from binary to text
file formats as computers have become more powerful (so reducing storage and
processing requirements becomes less valuable) and more networked (so increasing
interoperability becomes more valuable).

Another economically important difference is that a company can control use of
its binary file format through some combination of intellectual property rights
(e.g. patents) and simply limiting access to information about the format (trade
secrets). Controlling a *proprietary* file format in this manner can provide a 
competitive advantage to a software company. However, this advantage has become
less valuable over time as big companies demand the interoperability and
customization potential associated with open file formats and standards.
...
interaction among firms, you should consider taking a course in industrial
organization (e.g., SFU's third-year elective ECON 325).
:::
...
7. Save the file under a new name to create a working copy of this data set.
   - I would use the file name `TaxRates_WC.xlsx`.
...
The most common and useful general-purpose format for tabular data is called the
***comma separated values*** or ***CSV*** file format. A CSV file is just a text
...
Notice that the quotes around "Mary, Queen of Scots" are needed in order for the
comma to be interpreted as an actual comma rather than a delimiter between
cells. 
...
3. Select `CSV (Comma delimited) (*.csv)` from the drop-down box.
4. Enter the file name `CountryCodes.csv` in the text box.
5. Select `Save`.
   - You may get the warning message: "The selected file type does not support
     workbooks that contain multiple sheets. To save only the active sheet click
     OK...." If so, select `OK`.
6. Close Excel.
   - You may get another warning message: "Want to save your changes to 
     CountryCodes.csv?" If so, select `Don't save`.
...
something like this:  
![CountryCodes.csv viewed in a text editor](bin/CSVinNotepad.png){width=50%} 
:::

If a CSV file has `.csv` at the end of its file name, you can open it in Excel
by just double-clicking on it. Otherwise, you can use the Text Import Wizard.
...
the comma to delimit cells. 
 ...
 - ***Space-delimited*** text files are just like CSV files, but use spaces as
   the delimiter.  For example, a space-delimited version of our table might
   look like this:
 ...
 - ***Tab-delimited*** text files are just like CSV files, but use a tab
...
tools, but you can select `Data` from the menu bar and look around to see what
is available.
...
The data that we wish to analyze often comes in multiple tables from multiple
sources.  In order to proceed with the analysis, we often need to combine
...
This section will go through the most important tools and techniques of
combining data in these ways.
...
*TaxRates_WC.xlsx*. To combine them in a single file:
...
3. Make a copy of the *TaxRates* worksheet, and name it *GrowthData*. This sheet
   will be the starting point for the linked data that we will construct in the
   next two sections.
4. Save your file.
...
or more data sets.  In order to do this, we need to ***match*** or ***link***
observations in the two tables on the basis of one or more ID variables or
***keys***. 
...
For example, the formula `=XLOOKUP("California",A1:A10,C1:C10)` looks for a
...
:::example
**Matching with a crosswalk table**

The *TaxRates* worksheet from the Tax Foundation and the *Data* worksheet from 
the Penn World Table both provide information at the country level. But they do
not yet have a common key variable: 
...
names, so people who work with data usually prefer standardized codes.

One option is to simply change the country names in one of our tables, but
that goes against our general principle that we avoid changing data. A better
solution is to use a ***crosswalk table*** that gives the country code for each
country name, including name variations like "Republic of Korea" and
"South Korea."  The *CountryCodes* worksheet is a crosswalk table I have created
for this purpose. If you take a look at it, you will notice that there are
observations for both "Republic of Korea" and "South Korea."
...
3. Construct the appropriate formula in cell A2:
   - `lookup_value` should be the country name of the current observation
     (`B2`).
   - `lookup_array` should be the full list of country names
     (`CountryCodes!A2:A185`).
   - `return_array` should be the full list of country codes
     (`CountryCodes!B2:B185`).
...
6. Check your work: Column A should now display the ISO country code for each
   observation.
7. Save your file.
:::
...
(**pop** in the *Data* worksheet) in 2019.  Since we need to match both country
and year, we will need to use the `CONCAT()` function to put the country and
year together in a single criterion.
...
2. Enter the correct formula in cell K2:
   ...
   - `lookup_array` should be the full range of the **CountryYear** variable in
     the *Data* worksheet (`Data!A2:A12811`).
   - `return_array` should be the full range of the **pop** variable in the
     *Data* worksheet (`Data!H2:H12811`).
3. Make the cell references absolute where needed, which will result in the
   formula `=XLOOKUP(CONCAT(A2,"2019"),Data!A$2:A$12811,Data!H$2:H$12811)`.
4. Copy/paste or fill the formula to the remaining cells in column K.

We can also create another variable whose variable is the country's population
in 1990:

5. Enter **Pop1990** in cell L1 of *GrowthData* to name the variable.
6. Enter the correct formula in cell L2. It will be nearly identical to the
   formula in cell K2, but with "2019" replaced by "1990":
   `=XLOOKUP(CONCAT(A2,"1990"),Data!A$2:A$12811,Data!H$2:H$12811)`
7. Copy/paste or fill the formula to the remaining cells in column K.
8. Save your file.
...
have data going back to 1950, especially those countries that were part of or
...
a set of older functions `COUNTIF()`, `AVERAGEIF()`, `SUMIF()`, `MINIF()` and
`MAXIF()` that allow only a single criterion.  You may see these functions in
older worksheets.
:::
...
Excel will return an ***error code*** to indicate what has gone wrong. Excel's
...
| `#REF!`   | Reference to cell that has been deleted |                       |
...
5. Go to cell K43, and (try to) enter `-1`.  You will see this dialog box
...
turned on. This feature will help avoid problems in the first place.  In
addition, the Data Validation tool allows you to identify previously-entered
...
One of the most common Excel problems occurs when someone who is analyzing a
data set unintentionally changes the data. 

- This can happen because you accidentally touch your keyboard and overwrite
...
This is to some extent an unavoidable consequence of Excel's fundamental design:
there is no separation between data and analysis.  This design feature has the
positive implication that your data is always visible and accessible, but it
comes at a cost.
...
3. Any locked cell in any protected sheet cannot be edited.

You can change the status of any worksheet, or of any cell in an unprotected
worksheet.
...
prevent changes to that worksheet:
...
3. Lock columns A through N by selecting them, and then selecting 
...
Finally, you can remove protection from any worksheet by simply selecting 
`Home > Format > Unprotect Sheet`.

## Finishing up {#finishing-up-advanced-data-cleaning}

As always, be sure to save your working copy and update/create the master
version of your cleaned data file.
...
In the next few chapters, we will continue to develop applied methods. Our focus
will shift to [learning R](#introduction-to-r) for both
[data cleaning and analysis](#using-r), but we will also learn how to create
[pivot tables](#pivot-tables) in Excel.
