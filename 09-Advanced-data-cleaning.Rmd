...
In this chapter, we will build a data set describing long-run economic growth in
a wide cross-section of countries. In doing so, we will encounter some more
advanced data cleaning techniques such as merging and aggregation.  We will
also learn some advanced Excel functions and tools for data management.
...
The current version of the PWT is available at 
http://www.ggdc.net/pwt and is built from two distinct data sources:
...
each country's GDP data to comparable (PPP) units.
...
The data on the top marginal tax rate is obtained from the
Tax Foundation[^801], 
...
Finally, we have a "crosswalk" data file `CountryCodes.txt`that I have constructed
to help with linking the GDP and tax data (more about this below). 
:::
...
6. Enter the correct formula in cell L2. It will be nearly identical
...
looks like this: ![Data Validation button](bin/DataValidationButton.png){height=16pt}. 
...
data validation. To test it:

5. Go to cell K43, and (try to) enter `-1`.  You will see this dialog
...
5. To remove the circles, select `Data > Data Validation > Clear Validation Circles`.
:::
...
validation circles (Excel will no longer show any).
:::
...
cells.  Protecting and locking work according to three rules:
...
[data cleaning and analysis](#using-r), but we will also learn how to
create [pivot tables](#pivot-tables) in Excel.
