...
Every random variable is a number, so its sample space is the set of real
numbers $\mathbb{R}$. In some cases we *could* define its sample space more
narrowly, but it is simpler if we avoid doing so.
...
- If $z = 0.5$, then $x$ is exactly half of a standard deviation above its mean.
- If $z = -3$, then $x$ is exactly three standard deviations below its mean.
...
In principle, the set of possible probability distributions is unlimited.
However, some probability distributions are common enough in applications that
we have given them names. This provides a quick way to describe a particular
...
Using a standard distribution also allows us to find the properties of a
