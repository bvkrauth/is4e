...
- The player's net profits from a \$1 bet on red:
...
- The player's net profits from a \$1 bet on 14:
...
from the probability distribution of $\omega$:
   ...
   - The probability distribution for $b$ is:
      ...
      $$\Pr(b = 37) = \Pr(\omega \notin \{0,1,\ldots,36\}) = 0$$
   - The probability distribution for $y_{Red}$ is:
      $$\Pr(y_{Red} = 1) = \Pr(\omega \in RedWins) = 18/37 \approx 0.486$$
      $$\Pr(y_{Red} = -1) = \Pr(\omega \notin RedWins) = 19/37 \approx 0.514$$
      $$\Pr(y_{Red} \notin \{-1,1\}) = 0$$
...
The sample space of $y_{Red}$ is $\mathbb{R}$ and the support of
$y_{Red}$ is $S_{Red} = \{-1,1\}$.
...
Our three random variables are all discrete, and each has its own PDF:
  ...
  $$f_b(a) = \Pr(b = a) = \begin{cases}
            ...
            0 & \textrm{otherwise} \\
          \end{cases}$$
  $$f_{Red}(a) = \Pr(y_{Red} = a) = \begin{cases}
            19/37 & \textrm{if } a = -1 \\
            18/37 & \textrm{if } a = 1 \\
            0 & \textrm{otherwise}  \\
          \end{cases}$$
...
::: example
**Deriving the CDF of $y_{Red}$**

We will derive the CDF $F_{Red}(\cdot)$ of the random variable $y_{Red}$
...
1. Write down the formula, substituting in the appropriate variable names.
   In this case:
   \begin{align}
     F_{Red}(a) &= \Pr(y_{Red} \leq a) \\
      &= \sum_{s \in S_{Red}} f_{Red}(s)I(s \leq a)
   \end{align}
2. Find the support of the random variable, and substitute it into
   the formula.  In this case, we earlier found that the support of $y_{Red}$
   is $S_{Red} = \{-1,1\}$ so:
   \begin{align}
     F_{Red}(a) &= \sum_{s \in \{-1,1\} } f_{Red}(s)I(s \leq a)
   \end{align}
3. Expand out the summation:
   \begin{align}
     F_{Red}(a) &= f_{Red}(-1)I(-1 \leq a) + f_{Red}(1)I(1 \leq a)
   \end{align}
   If you don't know how to do this, review the material on summations
   in the Math Appendix.
4. Find the PDF of the random variable, and substitute it in.  In this
   case, we earlier found that $f_{Red}(-1) = 19/37 \approx 0.514$ and
   $f_{Red}(1) = 18/37 \approx 0.486$. Substituting in:
   \begin{align}
     F_{Red}(a) &= 19/37 * I(-1 \leq a) + 18/37 * I(1 \leq a) \\
      &\approx 0.514 * I(-1 \leq a) + 0.486 * I(1 \leq a) \\
   \end{align}
5. Pick out a few values for $a$, and plug them into the formula:
  \begin{align}
    F_{Red}(-100) &\approx 0.514 * \underbrace{I (-1 \leq -100)}_{=0} + 0.486 * \underbrace{I(1 \leq -100)}_{=0} \\
      &= 0 \\
    F_{Red}(-2) &\approx 0.514 * \underbrace{I (-1 \leq -2)}_{=0} + 0.486 * \underbrace{I(1 \leq -2)}_{=0} \\
      &= 0 \\
    F_{Red}(-1) &\approx 0.514 * \underbrace{I (-1 \leq -1)}_{=1} + 0.486 * \underbrace{I(1 \leq -1)}_{=0} \\
      &\approx 0.514 \\
    F_{Red}(0) &\approx 0.514 * \underbrace{I (-1 \leq 0)}_{=1} + 0.486 * \underbrace{I(1 \leq 0)}_{=0} \\
      &\approx 0.514 \\
    F_{Red}(1) &\approx 0.514 * \underbrace{I (-1 \leq 1)}_{=1} + 0.486 * \underbrace{I(1 \leq 1)}_{=1} \\
      &= 1 \\
    F_{Red}(100) &\approx 0.514 * \underbrace{I (-1 \leq 100)}_{=1} + 0.486 * \underbrace{I(1 \leq 100)}_{=1} \\
      &= 1 \\
  \end{align}
  You can try as many values as you like, until you get a feel for what this
  function looks like.
6. Summarize your results in as clear and simple a formula as you can.  Case
   notation is a good way to do this:
  \begin{align}
    F_{Red}(a) &\approx \begin{cases}
                  0 & a < -1 \\
                  0.514 & -1 \leq a < 1 \\
                  1 & a \geq 1 \\
                  \end{cases}
...
The player's net profit from a \$1 bet on red $(y_{Red})$ was earlier defined
directly from the underlying outcome $\omega$. We can use the indicator function
to write it as a nonlinear function of the random variable $b$:
  $$y_{Red} = 2I(b \in RedWins) - 1$$
or as a linear function of the random variable $r$:
  $$y_{Red} = 2r -1$$
:::
...
with each value weighted by the probability of observing that value. We can 
also think of it as a measure of "central tendency" (a typical or
representative value) for the random variable, or as a predicted value.

The formula for the expected value may look difficult if you are not used to the
notation, but it is actually quite simple to calculate.
...
1. Rewrite the formula with the correct variable names:
   ...
   \end{align}
2. Find the suppot and substitute in:
   \begin{align}
      E(y_{14}) &= \sum_{a \in \{-1,35\}} a f_{14}(a)
   \end{align}
2. Expand the summation:
   \begin{align}
      E(y_{14}) &= -1 * f_{14}(-1) + 35 * f_{14}(35)
   \end{align}
4. Substitute the PDF values, and substitute them in:
   \begin{align}
...
result can be interpreted as the house advantage: the player can expect to lose 
approximately 2.7 cents per dollar bet on 14, and the house can expect to
gain approximately 2.7 cents per dollar bet on 14.
:::

We can follow this procedure to calculate the expected value of any discrete
random variable.
...
Suppose that $y = h(x)$ where $x$ is a discrete random variable with PDF
$f_x(\cdot)$ and $h(\cdot)$ is some known function.  We can find the expected
value of $y$ using the formula:
\begin{align}
  E(y) &= E(h(x)) \\
    &= \sum_{a \in S_x} h(a) f_x(a) \\
\end{align}
...
directly from the PDF:
    \begin{align}
      E(y_{Red}) &= -1*\underbrace{f_{Red}(-1)}_{19/37} + 1*\underbrace{f_{Red}(1)}_{18/37} \\
        &\approx -0.027
    \end{align}
...
We also showed we can define $y_{Red}$ as a nonlinear function of $b$:
  $$y_{Red} = 2 I(b \in RedWins) - 1$$
Can we take the expected value inside this function?  That is, does:
  \begin{align}
    E(y_{Red}) &= 2 I(E(b) \in RedWins) - 1 \qquad \textrm{?} \\
  \end{align}
We already showed that $E(y_{Red}) \approx -0.027$. We also showed earlier that
$E(b) = 18$, so we can find:
  \begin{align}
    2 I(E(b) \in RedWins) - 1 &= 2 I(18 \in RedWins) - 1 \\
      ...
      &= 1
  \end{align}
Since $-0.027 \neq 1$, it is clear that $E(y_{Red}) \neq 2 I(E(b) \in RedWins) - 1$.
:::
...
The median of $y_{Red}$ is just its 0.5 quantile or 50th percentile:
  $$median(y_{Red}) = F_{Red}^{-1}(0.5) = -1$$
:::
...
The variance of $r$ is:
  \begin{align}
    var(r) &= (0-\underbrace{E(r)}_{18/37})^2 *\frac{19}{37} + (1-\underbrace{E(r)}_{18/37})^2 * \frac{18}{37} \\
      &\approx 0.25
  \end{align}
The variance of $y_{Red}$ is:
  \begin{align}
    var(y_{Red}) &= (-1-\underbrace{E(y_{Red})}_{\approx 0.027})^2 * \frac{19}{37} + (1-\underbrace{E(y_{Red})}_{\approx 0.027})^2 * \frac{18}{37} \\
    &\approx 1.0
  \end{align}
...
1. Calculate $E(y_{14})$ from the PDF. We found this earlier:
   \begin{align}
      E(y_{14}) = -0.027
2. Calculate $E(y_{14}^2)$ from the PDF:
  \begin{align}
    E(y_{14}^2) &= (-1)^2 f_{14}(-1) + 35^2 f_{14}(35) \\
      &= 1 * \frac{36}{37} + 1225 * \frac{1}{37} \\
      &\approx 34.08
  \end{align}
4. Put these results into the formula:
   \begin{align}
...
Notice that a bet on 14 has the same expected net profit as a bet on red:
  $$E(y_{14}) = E(y_{Red}) = -0.027$$
but its net profit is much more variable:
  $$var(y_{14}) = 34.1 > 1.0 = var(y_{Red})$$
:::
...
distribution.  We can use the formula above or the Excel function `BINOMDIST()`
to calculate the probability he makes all 10 shots, or exacly 7 shots, or at
least 5 shots.
...
calculations by hand.  For example, Stephen Curry would be expected to make
$10*0.43 = 4.3$ of his 10 shots and score $3*4.3 = 12.9$ points.
...
In our roulette example, the random variable $b$ has a discrete uniform
distribution on $\Omega = \{0,1,\ldots,36\}$.
:::
