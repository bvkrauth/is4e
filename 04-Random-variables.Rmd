...
Suppose we want to find the 0.25, 0.5, and 0.75 quantiles of the random variable
$r$. We start by writing down the CDF:
\begin{align}
  F_{r}(a) = \begin{cases}
      0 & a < 0 \\
      ...
      1 & a \geq 1 \\ \end{cases}
\end{align}
Then we plot the CDF, as in Figure \@ref(fig:RouletteQuantiles). To find any
quantile $q$, we can apply the definition,or just need to find the value on the
graph where $F_{r}(\cdot)$ crosses $q$.
...
the red line at $a = 0$. By the same method, we can find that the 0.5 quantile
(50th percentile) is also zero.
...
ggplot(data = RouletteCDF, mapping = aes(x = a)) +
  ...
  geom_text(x = 20, 
            ...
            label = "F_r(a)", 
            col = "red") +
  geom_text(mapping=aes(x = 5.5, y = 0.25),
            label = "0.25 quantile = 0",
            col = "blue") +
  geom_text(x = 6.0, 
            y = 0.5, 
            label = "0.5 quantile = 0",
            col = "blue") +
  geom_text(x = 6.5, 
            y = 0.75, 
            label = "0.75 quantile = 1",
            col = "blue") +
  geom_segment(x=-10,xend=0,y=0.25,yend=0.25,col="blue",linetype = "dashed") +
  geom_segment(x=-10,xend=0,y=0.5,yend=0.5,col="blue",linetype = "dashed") +
  geom_segment(x=-10,xend=1,y=0.75,yend=0.75,col="blue",linetype = "dashed") +
  labs(title = "Cumulative distribution function (CDF)", 
