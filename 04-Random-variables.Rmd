...
We can derive the PDF or CDF of a function of a random variable directly
from the PDF of the original random variable.  For example, suppose
that:
  $$y = g(x)$$
for some function $g(\cdot)$.  Then the PDF of y is:
  $$f_y(a) = \Pr(y = a) = \Pr\left(x \in \{b: g(b)=a\}\right) = \sum_{b:g(b)=a} f_x(b)$$
and the CDF of y is:
  $$F_y(a) = \Pr(y \leq a) = \Pr\left(x \in \{b: g(b) \leq a\}\right) = \sum_{b:g(b) \leq a} f_x(b)$$
...
### Calculating expected values {#properties-of-the-expected-value}
