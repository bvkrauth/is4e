...
Let $\alpha$ be any number between zero and one.  Then the $\alpha$ ***quantile***
of a random variable $x$ is defined as:
  $$q_x(\alpha) = \min\{a: \Pr(x \leq a) \geq \alpha\} = \min\{a: F_x(a) \geq \alpha\}$$
...
(25th percentile) is:
  $$q_{red}(0.25) = \min\{a: \Pr(w_{red} \leq a) \geq 0.25\} = \min [-1,1] = -1$$
which is depicted by the lowest blue dashed line.

By the same method, we can find that the 0.5 quantile (50th percentile) is:
  $$q_{red}(0.5) = \min\{a: \Pr(w_{red} \leq a) \geq 0.5\} = \min [-1,1] = -1$$
and the 0.75 quantile (75th percentile) is:
  $$q_{red}(0.75) = \min\{a: \Pr(w_{red} \leq a) \geq 0.75\} = \min\{1\} = 1$$
:::

The quantile function $q_x(\cdot)$ is also called the ***inverse CDF*** because
it is obtained by "flipping" the axes of the CDF.
