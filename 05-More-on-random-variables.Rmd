...
The first implication of this insight is that we can derive an alternative
...
The next two implications are that the order does not matter:
  $$cov(x,y) = cov(y,x)$$
and that the variance is just the covariance of a random variable 
with itself:
  $$cov(x,x) = var(x)$$
These follow from the corresponding properties of the product, since
$x*y = y*x$ and $x^2 = x*x$.

The next implication is that the *sign* of the covariance tells us the 
...
These two results can be combined in various ways, for example:
  \begin{align}
    cov(ax + by + c,dx + ey + f) &= ad var(x) + be Var(y) + (ae + bd) cov(x,y) 
  \end{align}
Finally, we can combine these results with our knowledge that
the variance is just the covariance of a variable with itself
to get:
