...
ggplot(data=NormalDist,mapping=aes(x=a,y=fa)) +
  ...
  labs(title = "Probability density function (PDF)", 
       ...
       tag = "")
```
The $N(0,1)$ distribution is symmetric around $\mu=0$ and bell-shaped,
meaning that it is usually close to zero but can occasionally be
quite far.  For other values of $\mu$, the $N(\mu,\sigma^2)$ distribution
is also symmetric around $\mu$ and bell-shaped, with the "spread"
of the distribution depending on the value of $\sigma^2$.

The formula for the normal PDF looks strange, but it turns out to be a very 
...
PDF or CDF of the normal distribution, and the Excel function
`NORM.INV()` can be used to calculate its quantile (inverse CDF)
function.
