...
ost of the results and intuition we have already developed for discrete 
...
There are several standard continuous distributions that are in regular 
usage.  The two most important are the uniform distribution and 
...
which looks like this:
```{r UniformPDF, fig.cap = "*PDF for the uniform distribution*"}
ggplot(data=UniformDist,mapping=aes(x=a,y=fa)) +
  geom_step(col = "blue") +
  geom_text(x=1.25,y=0.8,col="blue",label="f_x(a)") +
  xlab("a") +
...
The CDF of the uniform distribution looks like this: 
```{r UniformCDF, fig.cap = "*CDF for the uniform distribution*"}
UniformDist <- tibble(a=seq(from=-2,to=2,length.out=100),
                      Fa=punif(seq(from=-2,to=2,length.out=100)),
                      fa=dunif(seq(from=-2,to=2,length.out=100)))
ggplot(data=UniformDist,mapping=aes(x=a,y=Fa)) +
...
The mean and median of a uniform random variable is 

If $x \sim Uniform(L,H)$ then its median is just the midpoint of its 
support:
  $$Med(x) = E(x) = \frac{L+H}{2}$$
Its mean and variance can be found using intgral calculus, and are:
  $$E(x) = \frac{L+H}{2}$$
and:
  $$var(x) = \frac{(H-L)^2}{12}$$
Again, I do not expect you to be able to derive the mean and variance
here.
