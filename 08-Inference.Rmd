...
With our proposed critical values $(c_L,c_H) = (45,55)$, the probability of
rejecting the null is:
\begin{align}
  \Pr(\textrm{reject $H_0$}) &= \Pr((t_n < 45) \cup (t_n > 55)) \\
    ...
    &= F_t(44) + (1 - F_t(55))
\end{align}
where $F_t(\cdot)$ is the CDF of $t_n$. We know this CDF when the null is true:
\begin{align}
  H_0 &\implies t_n \sim Binomial(100,18/37)
\end{align}
and we can calculate the binomial CDF in Excel using the `BINOM.DIST()`
function.  The correct formulas are 
`=BINOM.DIST(44,100,18/37,TRUE)` and `=BINOM.DIST(55,100,18/37,TRUE)`
which produces:
\begin{align}
  \Pr(\textrm{reject $H_0$}) 
    &= F_t(44) + (1 - F_t(55)) \\
...
With our proposed critical values $(c_L,c_H) = (45,55)$, the probability of
rejecting the null is as we have already derived:
\begin{align}
  \Pr(\textrm{reject $H_0$}) &= F_t(44) + (1 - F_t(55))
\end{align}
When the null is false:
\begin{align}
  H_1 &\implies t_n \sim Binomial(100,p_{red}) \qquad \textrm{where $p_{red} \neq 18/37$}
\end{align}
The power of the test depends on the unknown true value of $p$, but we can
calculate the power for any value of $p_{red}$. For example, let's pick
$p_{red}=0.4$. Then:
\begin{align}
  power(0.7) &= F_t(44) + (1 - F_t(55)) \qquad \textrm{where $t_n \sim Binomial(100,0.4)$}
\end{align}
We can calculate these probabilities in Excel with the formulas:
`=BINOM.DIST(44,100,0.4,TRUE)` and `=BINOM.DIST(55,100,0.4,TRUE)`
which produces:
\begin{align}
  \Pr(\textrm{reject $H_0$}) 
    &= F_t(44) + (1 - F_t(55)) \\
    ...
    &\approx 0.822
\end{align}
That is, if the true win probability is 40\%, the probability of rejecting the
null of a fair game is about 82\%.
:::
...
If critical values are set to achieve a fixed size, the only way to get
increased power is by collecting more data. ***Power analysis*** is often used b
researchers to determine how much data to collect.  Each additional observation 
collected increases power without increasing size, but each additional
observation costs money. With limited resources, it is important to spend enough
to get clear results, but not much more than that.
