...
ggplot(data = PowerCurveData,
       mapping = aes(x = theta,y = power100)) +
  ...
  labs(title = "Power curve for test of fair roulette wheel", 
       ...
       tag = "")
```
Most power curves look like this graph: power is low for values close to the
null, and rises as we get further from the null. Power is a probability, so
...
The appropriate critical range depends on 
...
increase power is by collecting more data. ***Power analysis*** is often used
by researchers to determine how much data to collect.  Each additional
observation collected increases power without increasing size, but each
additional observation costs money. With limited research funding, it is
...
a smaller sample of size $n=20$. There are a few features I would like you to
notice, all of which are common to most regularly used tests:
...
ggplot(data = PowerCurveData, 
  ...
  geom_text(label="power=0.05",x=0.1,y=0.1,col="gray") +
...
*Case 1*: Red wins in 35 of 100 games, implying that $\bar{x} = 0.35$ and
...
*Case 2*: Red wins in 40 of the 100 games, implying that $\bar{x} = 0.40$ and
...
ggplot(data=simdata,
  ...
  xlab("value") +
  ylab("PDF of t_n") +
  labs(title = "Asymptotic distribution (PDF) of t_n under null", 
       subtitle = "", 
       caption = "", 
       tag = "")
```
...
$t_n \approx -2.84$. This is *outside* of the critical range, so we *reject* the
null of a fair game.

*Case 2*: Red wins 40 of the 100 games, and the test statistic is
$t_n \approx -1.75$. This is *inside* of the critical range, so we 
*fail to reject* the null of a fair game.
:::
...
In our roulette data, $x_i$ has a discrete support and could not possibly
be normally distributed. But we will ignore that for the moment.
:::
...
ggplot(data = simdata,
  ...
  xlab("value") +
  ylab("PDF") +
  labs(title = "Exact distribution (PDF) of t_n under null", 
       subtitle = "", 
...
$t_n \approx -2.84$. This is *outside* of the critical range, so we *reject*
the null of a fair game.
