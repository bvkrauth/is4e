...
It also provides the mathematical framework for the statistical analysis
of data.
...
for this random process, which is a function $Pr(\cdot)$ that assigns
a number $\Pr(A)$ to each possible event $A$.  The number is called 
...
The events "ball lands on zero" and "ball lands on 14"
are disjoint:
  $$\{0\} \cap \{14\} = \emptyset$$
so Axiom 3 applies and the probability that the ball lands
on zero or 14 is:
  $$\Pr(\{0\} \cup \{14\}) = \Pr(\{0\}) + \Pr(\{14\})$$
  
The events "ball lands on red" and "ball lands on 14"
are not disjoint:
  $$Red \cap \{14\} = \{14\}$$
...
- If we can know the probability that Red wins, we can use the complement rule
  ...
  $$\Pr(Red^c) = 1 - \Pr(Red)$$
- We know that 14 is a red number, so we can use the implied event rule
  to determine that:
  $$\Pr(\{14\}) \leq \Pr(Red)$$
- We earlier determined that "ball lands on red" and "ball lands on 14"
  are not disjoint, and so we could not apply Axiom 3 directly:
  $$\Pr(Red \cup \{14\}) \neq \Pr(Red) + \Pr(\{14\})$$
...
Asssuming each elementary event has the same probability, let that probability be:
...
real numbers, and can take on any of an infinite number of values. We will
learn more about those cases in Chapter \@ref(random-variables).
...
they are related. For example:
 ...
 - Political analysts often want to predict the winner of an election 
   based on partial results.
 - Financial analysts often want to forecast a company's earnings under different
   market scenarios.
...
the probability of some other event. This section will develop
some tools for dealing with this scenario.
...
and derived rules of probability described in Section \@ref(probabilities).
...
of testing negative:
   $$\Pr(T^c|D^c) = q$$
and the *prevalence* of the disease is the probability that
a given patient has it:
   $$\Pr(D) = d$$
...
probability: events, outcomes, joint/conditional/marginal probabilities,
and independence.  We have also learned various tools for calculating
probabilities, including the axioms of probability, their associated rules,
the law of total probability, and Bayes' law.  You may have seen many of these
terms and ideas in high school, but we are approaching them at a higher level.
Be sure to review these terms and concepts in detail, and do the practice
...
18. During World War II, the statistician Abraham Wald worked in a research
    ...
    - A plane is equally likely to be hit in the engine
      or on the body:
