...
1. ***Non-negativity***: Probabilities are never negative:
   $$\Pr(A) \geq 0$$
2. ***Unit measure***: One of the outcomes in the sample space will definitely
   happen:
   $$\Pr(\Omega) = 1$$
...
- ***Complement rule***: The probability of an event *not* happening is:
   $$\Pr(A^C) = 1 - \Pr(A)$$
- ***Empty event rule***: The probability of nothing happening is:
   $$\Pr(\emptyset) = 0$$
- ***Implied event rule***: Probabilities of implied events are larger:
...
- If we can know the probability that Red wins, we can use the complement rule
  ...
  $$\Pr(Red^c) = 1 - \Pr(Red)$$
- We know that $\{14\} \subset Red$, so we can use the implied event rule
  to determine that:
  $$\Pr(\{14\}) \leq \Pr(Red)$$
