...
[Functions](#functions) in the [Math Review](#math-review) appendix.
...
players. The house advantage is typically quite small - less than 5\% in many
...
Like the outcome, the sample space is something we choose.  We could have
defined the sample space for the game as the set of all positive integers, all
integers, or all real numbers.  As long as the sample space includes the 37
specific numbers in $\{0,1,2,\ldots,36\}$, we will be OK.
:::
...
- Ball lands on one of the first 12 numbers:
  ...
  \end{align}
- Ball lands on any number:
  \begin{align}
    \omega \in \Omega = \{0,1,2,\ldots,36\}
  \end{align}
...
1. ***Non-negativity***: The probability of an event cannot be negative:
  \begin{align}
    \Pr(A) \geq 0
  \end{align}
2. ***Unit measure***: The sample space includes all possible outcomes:
  \begin{align}
    \Pr(\Omega) = 1
  \end{align}
...
- ***Complement rule***: The probability of an event *not* happening is:
  \begin{align}
    \Pr(A^C) = 1 - \Pr(A)
  \end{align}
- ***Empty event rule***: The probability of an empty (impossible) event is
  zero:
  \begin{align}
    \Pr(\emptyset) = 0
  \end{align}
- ***Implied event rule***: An event cannot have higher probability than another
   event it implies:
   \begin{align}
...
- The event "Red wins" and the event "Black wins" are disjoint
  $(RedWins \cap BlackWins = \emptyset)$; it is not *logically* possible for both
  red and black to win. The empty event rule implies that the *probability* that
  red and black both win is:
  \begin{align}
    \Pr(RedWins \cap BlackWins) &= \Pr(\emptyset) \\
      &= 0
  \end{align}
...
- We earlier determined that the events $RedWins$ and $14Wins$ are not
  ...
  \begin{align}
    \Pr(RedWins \cup 14Wins) &= \Pr(RedWins) + \Pr(14Wins) \\
        & - \Pr(RedWins \cap 14Wins)
  \end{align}
:::
...
probability calculations can be made in three simple steps:

1. List the outcomes (elementary events) that imply our event of interest.
2. Calculate the probability of each outcome, based on information or
   assumptions about the random process.
3. Add up the probabilities.

The reason this works is because the elementary events are disjoint, so we can
apply the additivity axiom.

We can summarize this procedure with a formula:
\begin{align}
  \Pr(A) &= \sum_{s \in A} \Pr(\omega = s)
\end{align}
or equivalently:
\begin{align}
  \Pr(A) &= \sum_{s \in \Omega} \Pr(\omega = s)I(s \in A)
\end{align}
...
in the Math Review Appendix. These formulas might look intimidating, but they
...
The assumption of fairness implies that each elementary event has the same
probability, so let that probability be:
\begin{align}
  p = \Pr(\omega = 0) = \Pr(\omega = 1) = \cdots = \Pr(\omega = 36)
\end{align}
We can then use the axioms of probability to find the correct value of $p$:
...
In our roulette example, all elementary events have the same probability. Is
this the case for all random processes? No.
...
world sprinting champion Usain Bolt. There are two possible outcomes:

- I win the race
- Usain Bolt wins the race

but it would be unreasonable to think that these two
outcomes have equal probability.
:::

Once we have the probabilities of each elementary event, any other probability
can be calculated by simple addition.
...
We are often interested in how one event is related to another. For example:
 ...
 - Poker or blackjack players can place additional bets after viewing their
   cards. So they want to predict their chances of winning based on the cards
   that they have.
 - News organizations want to predict the winner of an election before all
   votes have been counted.
 - Financial analysts want to forecast a company's earnings under
   different market scenarios.
 - Economists want to predict future economic conditions based on data
   about current conditions.
 - Doctors want to predict the effectiveness of a particular treatment
   based on test results and patient characteristics.

In all of these cases, we know that a particular random event has happened and
want the probability of some other random event. This section will develop tools
...
\begin{align}
  \Pr(AlWins|BettyWins) &= \Pr(RedWins | EvenWins) \\
    ...
    &\approx 0.444
\end{align}
That is, Al is less likely to win if Betty wins (since $0.444 < 0.486$).
:::
...
\begin{align}
  \Pr(B|A) = \Pr(B)
\end{align}
When two events are independent, knowing that one event is true tells you
nothing about whether the other event is true.
...
Are these events independent? We have two ways of answering this question:

1. Compare joint probability with product of individual probabilities:
   - We calculated the joint probability in an earlier example:
     \begin{align}
       \Pr(RedWins \cap EvenWins) = 8/37 \approx 0.216
     \end{align}
   - The product of the individual probabilities is:
     \begin{align}
       \Pr(RedWins) \times \Pr(EvenWins) = 18/37 \times 18/37 \approx 0.237
     \end{align}
   - Since $0.216 \neq 0.237$, these two events are *not* independent.
2 .Compare conditional and unconditional probabilities:
   - We calculated the conditional probability in an earlier example:
     \begin{align}
       \Pr(RedWins|EvenWins) = 8/18 \approx 0.444
     \end{align}
   - We also calculated the unconditional probability:
     \begin{align}
       \Pr(RedWins) = 18/37 \approx 0.486
     \end{align}
    - Since $0.44 \neq 0.486$, these two events are *not* independent.

Either of these methods will work, so which one you use is strictly a matter of
convenience.
:::
...
Suppose you are playing [Texas hold'em poker](https://en.wikipedia.org/wiki/Texas_hold_%27em)
with a few friends, and the hand has one card left to deal (the "river").

- If the last card has a heart on it (event $Hearts$, 25\% probability) you will
  have a flush and win the hand with a probability you estimate to be 90\%.
- If the last card does not have a heart on it (event $Hearts^c$, 75\%
  probability), you will not have a flush, but still have a win probability
  you estimate to be 20\%.
...
\begin{align}
  \Pr(Win) &= \Pr(Win|Hearts)\Pr(Hearts) 
    ...
    &= 0.375
\end{align}
So you have a 37.5\% chance of winning.
:::
...
*Case #1*: Suppose the disease itself is fairly common ($prev = 0.10$).  Then a
patient who tests positive is likely to have the disease:
\begin{align}
  \Pr(Sick|Positive) = \frac{1*0.1}{1*0.1 + (1-0.99)*(1-0.1)} \approx 0.917
\end{align}
   
*Case #2*: Suppose the disease itself is quite rare ($prev = 0.001$).  Then even
a patient who tests positive is unlikely to have the disease:
\begin{align}
...
Most of the practice problems for this chapter are based on the casino game of [craps](https://en.wikipedia.org/wiki/Craps).

Craps is played with a pair of 6-sided dice. Players take turns rolling the
dice, and the player currently rolling the dice is called the "shooter".  There
are various bets - pass, don't pass, come, don't come, field, place, buy - that
can be placed on the results of multiple rolls of the dice.  These bets and
their probability calculations can be quite complex, so we will focus on
a few "single roll" bets:
...
2. Using enumeration, define the following events:
   a. Yo wins ($YoWins$).
   b. Snake eyes wins ($SnakeEyesWins$).
   c. Boxcars wins ($BoxcarsWins$).
   d. Field wins ($FieldWins$).

**GOAL #2: Use set theory to define and manipulate events**

3. Which of the following statements are true?
   a. The events $YoWins$ and $BoxcarsWins$ are identical.
   b. The events $YoWins$ and $(r,w) = (5,6)$ are identical.
   c. The events $BoxcarsWins$ and $(r,w) = (6,6)$ are identical.

4. Which of the following statements are true?
   a. The events $YoWins$ and $BoxcarsWins$ are disjoint.
   b. The events $YoWins$ and $FieldWins$ are disjoint.
   c. The events $YoWins$ and $BoxcarsLoses$ are disjoint.
   d. The events $YoWins$ and $FieldLoses$ are disjoint.

5. Which of the following statements are true?
   a. The event $YoWins$ implies the event $BoxcarsWins$.
   b. The event $YoWins$ implies the event $BoxcarsLoses$.
   c. The event $YoWins$ implies the event $FieldWins$.
   d. The event $YoWins$ implies the event $FieldLoses$.
...
13. Calculate each of the following joint probabilities:
    a. $\Pr(YoWins \cap BoxcarsWins)$
    b. $\Pr(YoWins \cap FieldWins)$
    c. $\Pr(YoWins \cap BoxcarsLoses)$

14. Calculate each of the following conditional probabilities:
    a. $\Pr(YoWins | BoxcarsWins)$
    b. $\Pr(YoWins | FieldWins)$
    c. $\Pr(YoWins | BoxcarsLoses)$
    d. $\Pr(FieldWins | YoWins)$
    e. $\Pr(BoxcarsWins | YoWins)$
...
17. The main bet in craps is a multi-roll bet called "pass."  The first roll is
    ...
    Let $c$ be the value of the come out roll, and let the event $PassWins$
    include every outcome in which Pass wins.  The table below reports both the
    probability of each value of the come out roll, and the probability that
    pass wins for each value of the come out roll.

    |Come out ($c$) | $\Pr(c)$ | Win probability $\Pr(PassWins|c)$ |
    |:--------------|:--------:|:---------------------------------:|
    |  2            |   1/36   |      0                            |
    |  3            |   2/36   |      0                            |
    |  4            |   3/36   |    3/9                            |
    ...
    Use the Law of Total Probability to calculate $\Pr(PassWins)$, the
    probability that a bet on Pass wins.
...
18. During World War II, the statistician Abraham Wald worked in a research
    ...
    Suppose that planes can be hit on the engine (event $EngineHit$) or on the
    body (event $BodyHit$)^[Note that it is possible for a plane to be hit in
    both places, or to not be hit at all.  So these two events are not
    necessarily disjoint, complements, or independent.] and that planes can
    either return safely (event $Return$) or crash (event $Return^C$). In
    addition, suppose that:

    - A plane is equally likely to be hit in the engine or on the body:
      \begin{align}
        \Pr(EngineHit) = \Pr(BodyHit) = 0.6
      \end{align}
    - Many of the returning planes show body damage:
      \begin{align}
        \Pr(BodyHit|Return) = 0.7
      \end{align}
    - Few of the returning planes show engine damage:
      \begin{align}
        \Pr(EngineHit|Return) = 0.1
      \end{align}
    ...
    Based on this information, calculate $\Pr(Return|BodyHit)$ and
    $\Pr(Return|EngineHit)$.
