...
So we will briefly review the most important terminology and ideas.
...
Complements take elements out of the first set if 
...
Let $A$ and $B$ be two events.  Then our three basic rules
...
::: example
**Conditional probabilties in roulette**

In our roulette example:
   $$\Pr(Red|Even) = \frac{\Pr(Red \cap Even)}{\Pr(Even)} = \frac{8/37}{18/37} \approx 0.444$$
   $$\Pr(b = 12|Even) = \frac{\Pr(b = 12 \cap Even)}{\Pr(Even)} = \frac{1/37}{18/37} \approx 0.056$$
:::
...
and $B$ that have nonzero[^1] probability.  Then by the definition of
independence:
...
[^1]: You may wonder: if it makes more sense to describe independence 
